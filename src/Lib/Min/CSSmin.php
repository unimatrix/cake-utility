...
// https://github.com/futtta/YUI-CSS-compressor-PHP-port/blob/master/cssmin.php @ 715d4d1 on Jan 22 2016
...
class CSSmin {
    ...
    /**
     *
     * @param bool|int $raise_php_limits If true, PHP settings will be raised if needed
     */
    ...
    private function minify($css, $linebreak_pos) {
        ...
        $css = preg_replace_callback('/flex\s?:\s?((?:[0-9 ]*)\s?(?:px|em|auto|%)?(?:calc\(.*\))?)/i', array(
                $this,
                'replace_flex'
        ), $css);
